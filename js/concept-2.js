(function(){
  ...
  if(canvas){
    ...
    function draw(time){ctx.clearRect(0,0,w,h);var t=time*.00018;var gradient=ctx.createRadialGradient(w*pointer.x,h*pointer.y,0,w*pointer.x,h*pointer.y,w*.62);gradient.addColorStop(0,'rgba(185,154,95,.22)');gradient.addColorStop(.35,'rgba(76,66,91,.14)');gradient.addColorStop(1,'rgba(7,8,9,0)');ctx.fillStyle=gradient;ctx.fillRect(0,0,w,h);for(var band=0;band<7;band++){ctx.beginPath();for(var x=-40;x<=w+40;x+=12){var phase=x/w*5.7+t*(band%2?1:-1)+band*.72;var centre=h*(.35+band*.055);var y=centre+Math.sin(phase)*h*(.055+band*.006)+Math.cos(phase*.47+t)*h*.035+(pointer.y-.5)*35;if(x===-40)ctx.moveTo(x,y);else ctx.lineTo(x,y)}ctx.strokeStyle=band===3?'rgba(220,202,164,.18)':'rgba(185,190,199,'+(0.035+band*.012)+')';ctx.lineWidth=band===3?1.2:.7;ctx.stroke()}if(!reduce)requestAnimationFrame(draw)}
